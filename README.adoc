...
That's it. The setup script checks prerequisites, installs dependencies, detects your AWS credentials and pull secret, verifies quotas, and prompts to deploy.
...
This installs the same prerequisites plus dev tools (shellcheck, nodejs) for content development.

== Quick Start

=== Prerequisites

* RHEL 9 / Fedora / macOS with Podman 5+, Python 3.12+, AWS CLI
* AWS sandbox from https://demo.redhat.com[demo.redhat.com] (Open Environment)
* OpenShift pull secret from https://console.redhat.com/openshift/downloads[console.redhat.com]
* `aws configure` run with your sandbox credentials
...
1. Check and install prerequisites (podman, git, jq, awscli, sshpass)
2. Clone AgnosticD v2 and install Ansible collections
3. Prompt for configuration (sandbox account, region, student count)
4. Auto-detect AWS credentials and pull secret
5. Validate the environment (6 checks)
6. Verify AWS quotas (vCPUs, Elastic IPs, VPCs)
7. Prompt to deploy when everything passes

=== Deploy Full Workshop (Automated)

After `make setup` completes, deploy Hub + Student clusters:
...
| `make setup`
| Full interactive onboarding (prod mode)

| `make setup-dev`
| Contributor/maintainer setup (dev mode)

| `make check`
| Run validation checks only (CI-friendly)

| `make quota`
| Check AWS quotas

| `make deploy`
| Deploy full workshop (Hub + Students)

| `make deploy-sno STUDENT=student1 ACCOUNT=sandbox5466`
| Deploy a single SNO cluster

| `make status STUDENT=student1 ACCOUNT=sandbox5466`
| Check cluster status

| `make destroy STUDENT=student1 ACCOUNT=sandbox5466`
| Destroy a cluster
|===
