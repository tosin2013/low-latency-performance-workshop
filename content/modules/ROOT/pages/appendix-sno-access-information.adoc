...
5. Enter password: (from `cat ~/kubeadmin-password` on bastion)
6. Click "Log In"
...
ssh {bastion_ssh_user_name}@{bastion_public_hostname} "cat ~/kubeadmin-password"
----
...
Username: `kubeadmin` +
Password: Run `cat ~/kubeadmin-password` on bastion
