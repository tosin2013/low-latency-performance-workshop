...
[IMPORTANT]
====
*🚀 Quick Start: SSH to Your Bastion Host (Recommended)*
...
If you prefer direct cluster access (e.g., from your local machine), use these credentials:
...
| Username | `kubeadmin`
| Password | Available on bastion at `~/ocp/auth/kubeadmin-password`
|===

[source,bash,role=execute,subs="attributes"]
----
# Direct login (from your local machine)
oc login {cluster_api} -u kubeadmin -p <password>
----

The kubeadmin password is available on the bastion at `~/ocp/auth/kubeadmin-password`.
====
...
link:{devspaces_factory}[🚀 *Open in Dev Spaces*,role=button]
...
* ✅ **OpenShift Virtualization** - Deployed and configured
* ✅ **SR-IOV Network Operator** - Deployed and configured
* ✅ **Node Tuning Operator** - Built-in (OpenShift 4.11+)
* ✅ **RHACM Integration** - Cluster imported to hub
...
* ✅ Node Tuning Operator: TuneD pods running, PerformanceProfile CRD available
* ✅ SR-IOV Network Operator: CSV in "Succeeded" phase
* ✅ OpenShift Virtualization: CSV in "Succeeded" phase, HyperConverged instance ready
...
✅ *Verified* your SNO cluster is imported into RHACM +
...
✅ *Understood* the GitOps architecture used for operator deployment (reference material) +
...
* ✅ SNO cluster imported and managed by RHACM
