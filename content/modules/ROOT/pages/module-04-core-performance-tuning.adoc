...
[NOTE]
====
**RT Kernel Installation (if enabled):**

If you enabled RT kernel, the node will reboot during the update process to load the real-time kernel. This is expected behavior and may take 10-20 minutes. The node will automatically rejoin the cluster after reboot.

If RT kernel is disabled (default), the update process is faster and does not require a reboot - only kernel parameters are adjusted.
====
...
if [ "${ENABLE_RT_KERNEL:-false}" = "true" ]; then
    ...
    echo "✅ Node update complete (no reboot required)"
fi
