...
# Get CPU information
CPU_COUNT=$(oc debug node/$TARGET_NODE -- chroot /host nproc)
echo "=== CPU Allocation Planning ==="
echo "Target node: $TARGET_NODE"
...
if [ "$CLUSTER_TYPE" = "SNO" ]; then
    echo ""
    echo "🔧 SNO CPU Allocation Strategy (Workshop-Friendly):"
    ...
    if [ $CPU_COUNT -gt 16 ]; then
        # For high CPU count (32+ cores): Reserve ~40% for system, isolate ~60%
        ...
        # For medium CPU count (16+ cores): Reserve ~50% for system, isolate ~50%
        ...
        # For lower CPU count (8+ cores): Reserve ~60% for system, isolate ~40%
        ...
        # For very low CPU count: Reserve most for system, minimal isolation
        RESERVED_CPUS="0-$((CPU_COUNT-2))"
        ISOLATED_CPUS="$((CPU_COUNT-1))"
        echo "   - Very low CPU count ($CPU_COUNT cores): Minimal isolation for demonstration only"
    fi

elif [ "$CLUSTER_TYPE" = "MULTI_NODE" ]; then
    echo ""
    echo "🔧 Multi-Node CPU Allocation Strategy (Workshop-Friendly):"
    ...
    if [ $CPU_COUNT -gt 8 ]; then
        # For high CPU count: Reserve ~25% for system, isolate ~75%
        ...
        # For medium CPU count: Reserve ~33% for system, isolate ~67%
        ...
        # For low CPU count: Reserve 50% for system
        RESERVED_CPUS="0-1"
    ...
    echo ""
    echo "🔧 Multi-Master CPU Allocation Strategy (Workshop-Friendly):"
    ...
    if [ $CPU_COUNT -gt 8 ]; then
        # For high CPU count: Reserve ~40% for control plane, isolate ~60%
        RESERVED_COUNT=$((CPU_COUNT * 40 / 100))
        ...
        # For medium CPU count: Reserve ~50% for control plane
        RESERVED_COUNT=$((CPU_COUNT / 2))
        RESERVED_CPUS="0-$((RESERVED_COUNT-1))"
        ISOLATED_CPUS="$RESERVED_COUNT-$((CPU_COUNT-1))"
        echo "   - Medium CPU count ($CPU_COUNT cores): Reserve $RESERVED_COUNT CPUs (~50%) for control plane"
    else
        # For low CPU count: Reserve most for control plane
        RESERVED_CPUS="0-$((CPU_COUNT-2))"
        ISOLATED_CPUS="$((CPU_COUNT-1))"
...
echo ""
echo "📊 Final CPU Allocation (Workshop-Optimized):"
echo "   Reserved CPUs: $RESERVED_CPUS"
echo "   Isolated CPUs: $ISOLATED_CPUS"

# Calculate and display percentages for clarity
RESERVED_COUNT=$(echo $RESERVED_CPUS | tr '-' ' ' | awk '{if(NF==2) print $2-$1+1; else print 1}')
ISOLATED_COUNT=$(echo $ISOLATED_CPUS | tr '-' ' ' | awk '{if(NF==2) print $2-$1+1; else print 1}')
...
echo ""
echo "✅ This allocation preserves cluster functionality while demonstrating performance benefits"

# Save CPU configuration
echo "RESERVED_CPUS=$RESERVED_CPUS" >> /tmp/cluster-config
echo "ISOLATED_CPUS=$ISOLATED_CPUS" >> /tmp/cluster-config
echo "RESERVED_COUNT=$RESERVED_COUNT" >> /tmp/cluster-config
echo "ISOLATED_COUNT=$ISOLATED_COUNT" >> /tmp/cluster-config
----
...
# Determine HugePages allocation based on cluster type
if [ "$CLUSTER_TYPE" = "SNO" ]; then
    HUGEPAGES_COUNT=1    # Conservative for SNO
    NODE_SELECTOR='node-role.kubernetes.io/master: ""'
    PROFILE_NAME="sno-low-latency-profile"
    echo "📝 SNO: Using conservative HugePages (1GB) and master node selector"
elif [ "$CLUSTER_TYPE" = "MULTI_NODE" ]; then
    HUGEPAGES_COUNT=2    # Standard for dedicated workers
    NODE_SELECTOR='node-role.kubernetes.io/worker-rt: ""'
    PROFILE_NAME="worker-low-latency-profile"
    echo "📝 Multi-Node: Using standard HugePages (2GB) and worker-rt selector"
else
    HUGEPAGES_COUNT=1    # Conservative for multi-master
    NODE_SELECTOR='node-role.kubernetes.io/master-rt: ""'
    PROFILE_NAME="master-low-latency-profile"
    echo "📝 Multi-Master: Using conservative HugePages (1GB) and master-rt selector"
fi

echo ""
echo "🚀 Creating Performance Profile: $PROFILE_NAME"

# Create the Performance Profile with dynamic configuration
cat << EOF | oc apply -f -
...
echo "PROFILE_NAME=$PROFILE_NAME" >> /tmp/cluster-config
----
...
if oc get performanceprofile $PROFILE_NAME >/dev/null 2>&1; then
    ...
    oc get performanceprofile $PROFILE_NAME -o jsonpath='{.metadata.name}{"\n"}CPU Reserved: {.spec.cpu.reserved}{"\n"}CPU Isolated: {.spec.cpu.isolated}{"\n"}HugePages: {.spec.hugepages.pages[0].count}x{.spec.hugepages.pages[0].size}{"\n"}RT Kernel: {.spec.realTimeKernel.enabled}{"\n"}'
    echo ""
    echo "🔍 Full Profile Configuration:"
    oc get performanceprofile $PROFILE_NAME -o yaml
else
    echo "❌ Performance Profile creation failed!"
...
The workshop provides additional validation scripts to verify specific aspects of your performance tuning configuration.
