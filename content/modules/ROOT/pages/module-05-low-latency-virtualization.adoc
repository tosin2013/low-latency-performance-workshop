...
[WARNING]
====
**Virtualized Instance Configuration (m5.4xlarge)**

This workshop uses AWS m5.4xlarge instances, which are virtualized EC2 instances (not bare-metal). 
OpenShift Virtualization is configured to run in **software emulation mode** (QEMU TCG) because 
hardware virtualization (KVM) is not available on virtualized instances.

**Performance Expectations:**

* **VM Boot Time**: VMs will boot successfully but will be slower than on bare-metal (typically 2-5 minutes vs. 30-60 seconds)
* **CPU Performance**: VMs will run 10-50x slower than native performance due to software emulation
* **HugePages**: Will work but provide less benefit than on bare-metal with KVM
* **CPU Pinning**: Will be configured but have limited effectiveness without hardware virtualization
* **Live Migration**: Not supported in emulation mode

**What Still Works:**

* All VM management features (create, delete, console access)
* Network configuration and multi-interface setups
* Storage and DataVolumes
* Performance optimizations (HugePages, CPU pinning) - configured but with reduced benefit
* Educational demonstrations of low-latency concepts

**Production Considerations:**

For production workloads requiring optimal VM performance, use bare-metal instances (e.g., `.metal` instance types) 
that support KVM hardware virtualization. This workshop configuration is designed for educational purposes 
to demonstrate OpenShift Virtualization concepts on accessible cloud infrastructure.

The HyperConverged CR is configured with `developerConfiguration.useEmulation: true` to enable this mode.
====
...
oc get nodes -o jsonpath='{.items[*].status.allocatable.devices\.kubevirt\.io/kvm}' | grep -q "1k" && echo "✅ KVM available on cluster nodes" || echo "❌ KVM not available"
  # Note: On virtualized instances (m5.4xlarge), KVM will not be available.
  # This is expected - OpenShift Virtualization will use software emulation mode.
...
echo "Creating Fedora VMI template for kube-burner testing"
  ...
  # Note: This template uses 'host-model' CPU mode for compatibility with virtualized instances
  # For bare-metal with KVM, you could use 'host-passthrough' for better performance
  # The module05-configure-vmi.sh script automatically detects the environment
...
* **Auto-detects virtualized vs bare-metal environment** (checks emulation mode)
* Generates optimized VMI YAML with:
  - CPU pinning (if Performance Profile exists)
  - HugePages (if available)
  - **Appropriate CPU model**:
    * `host-passthrough` - For bare-metal with KVM (best performance)
    * `host-model` - For virtualized instances (m5.4xlarge) or when KVM unavailable
...
----
  ...
  # This error occurs on virtualized instances (m5.4xlarge) because:
  # - host-passthrough requires KVM (hardware virtualization)
  # - Virtualized instances use software emulation (QEMU TCG) - no KVM available

  # Solution 1: Use the module05-configure-vmi.sh script (recommended)
  # It automatically detects virtualized instances and uses host-model
bash ~/low-latency-performance-workshop/scripts/module05-configure-vmi.sh

  # Solution 2: Manually check emulation mode and use host-model
EMULATION=$(oc get kubevirt -n openshift-cnv kubevirt-kubevirt-hyperconverged -o jsonpath='{.spec.configuration.developerConfiguration.useEmulation}' 2>/dev/null)
if [ "$EMULATION" = "true" ]; then
    echo "Virtualized instance detected - use host-model CPU mode"
    # In your VMI spec, use: model: host-model
else
    echo "Bare-metal instance - host-passthrough may work"
    # In your VMI spec, you can try: model: host-passthrough
fi

  # Solution 3: Use a specific CPU model for compatibility
