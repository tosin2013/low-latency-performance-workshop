#!/bin/bash
# Deploy a single user SNO cluster
#
# Usage: 
#   ./deploy-single-sno.sh [user_name] [mode]
#
# Examples:
#   ./deploy-single-sno.sh user1           # RHPDS mode (default, with hub integration)
#   ./deploy-single-sno.sh user1 rhpds     # RHPDS mode (explicit)
#   ./deploy-single-sno.sh user1 standalone # Standalone mode (no hub integration)

set -e

USER_NAME_RAW=${1:-user1}
...
USER_NAME=$(echo "${USER_NAME_RAW}" | tr '[:upper:]' '[:lower:]')
...
if [ "${USER_NAME}" != "${USER_NAME_RAW}" ]; then
    echo "⚠ User name converted to lowercase: ${USER_NAME_RAW} → ${USER_NAME}"
    echo "  (OpenShift cluster names must be lowercase per RFC 1123)"
    echo ""
fi
echo "User: ${USER_NAME}"
echo "GUID: workshop-${USER_NAME}"
...
if [ "${DEPLOYMENT_MODE}" == "rhpds" ]; then
    if ! oc whoami &> /dev/null; then
        ...
        echo "Or use standalone mode: ./03-test-single-sno.sh ${USER_NAME} standalone"
        exit 1
    fi
...
echo "Log file: /tmp/workshop-${USER_NAME}.log"
echo ""
...
WORKSHOP_USER_PASSWORD="${WORKSHOP_PASSWORD:-workshop}"
...
echo ""
eval "${ANSIBLE_NAVIGATOR_CMD}" 2>&1 | tee /tmp/workshop-${USER_NAME}.log
...
if [ -f "${WORKSHOP_DIR}/workshop-scripts/test-bastion-ssh.sh" ]; then
    ...
    if ${WORKSHOP_DIR}/workshop-scripts/test-bastion-ssh.sh ${USER_NAME}; then
        echo ""
...
KUBECONFIG_PATH=~/agnosticd-output/workshop-${USER_NAME}/kubeconfig
SSH_KEY_PATH=~/agnosticd-output/workshop-${USER_NAME}/ssh_provision_workshop-${USER_NAME}
...
if [ -f ${SSH_KEY_PATH} ]; then
    ...
    BASTION_IP=$(aws ec2 describe-instances \
      --region ${AWS_REGION} \
      --filters "Name=tag:guid,Values=workshop-${USER_NAME}" "Name=tag:Name,Values=bastion" "Name=instance-state-name,Values=running" \
...
if [ "${DEPLOYMENT_MODE}" == "rhpds" ]; then
    ...
    if oc get managedcluster workshop-${USER_NAME} &> /dev/null; then
        ...
        # Check status
        STATUS=$(oc get managedcluster workshop-${USER_NAME} -o jsonpath='{.status.conditions[?(@.type=="ManagedClusterConditionAvailable")].status}')
        if [ "${STATUS}" == "True" ]; then
        ...
        # Show details
        echo ""
        oc get managedcluster workshop-${USER_NAME}
    else
    ...
    if [ "${RHACM_IMPORT_SUCCESS}" == "false" ]; then
        ...
        MANUAL_IMPORT_SCRIPT="/tmp/manual-import-workshop-${USER_NAME}.sh"
...
MANAGED_CLUSTER_NAME="workshop-${USER_NAME}"
...
IMPORT_SCRIPT_EOF
        ...
        sed -i "s|USER_NAME_PLACEHOLDER|${USER_NAME}|g" ${MANUAL_IMPORT_SCRIPT}
        sed -i "s|SNO_KUBECONFIG_PLACEHOLDER|${KUBECONFIG_PATH}|g" ${MANUAL_IMPORT_SCRIPT}
        sed -i "s|GUID_PLACEHOLDER|workshop-${USER_NAME}|g" ${MANUAL_IMPORT_SCRIPT}
...
echo "GUID: workshop-${USER_NAME}"
echo "Mode: ${DEPLOYMENT_MODE}"
echo "Log: /tmp/workshop-${USER_NAME}.log"
...
if [ "${DEPLOYMENT_MODE}" == "rhpds" ]; then
    ...
    echo "  oc get managedcluster workshop-${USER_NAME}"
    echo ""
    
    # Mention manual import script if it was generated
    if [ "${RHACM_IMPORT_SUCCESS}" == "false" ] && [ -f "/tmp/manual-import-workshop-${USER_NAME}.sh" ]; then
        echo "⚠️  RHACM auto-import failed or incomplete"
        echo ""
        echo "To manually import the cluster:"
        echo "  /tmp/manual-import-workshop-${USER_NAME}.sh"
        echo ""
    fi
fi
...
if [ "${DEPLOYMENT_MODE}" == "rhpds" ]; then
    if [ "${RHACM_IMPORT_SUCCESS}" == "false" ]; then
        echo "  3. Run manual RHACM import: /tmp/manual-import-workshop-${USER_NAME}.sh"
